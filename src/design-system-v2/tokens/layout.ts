...
/**
 * **검색 줄과 그 바로 아래 가로 레일(카테고리) 사이.**
 *
 * 두 기능(레시피 탭 · 커뮤니티 탭)이 **같이 쓰는 값이라 여기 산다.** 2026-08-21 에
 * 사용자가 두 탭을 같은 문법으로 못 박았다 — 검색이 위, 카테고리가 그 밑, 둘 다
 * 스크롤에 영향받지 않는 고정층. 그 한 칸이 탭마다 다르면 "통일" 이라는 말이 거짓이 된다.
 * 기능 폴더 어느 한쪽에 두면 다른 쪽이 사본을 들게 되고, 사본은 언젠가 한쪽만 고쳐진다.
 *
 * **레일 자신의 세로 여백은 여기에 포함되지 않는다.** 칩까지의 광학적 거리는
 * 이 값 + 레일의 위 여백이다(커뮤니티 `CHIP_RAIL_INSET_V.results` 10 → 18,
 * 레시피 카테고리 캐러셀의 슬롯 위 여백 2 → 10). 레일의 몫을 여기서 다시 세면
 * 레일이 밀도를 바꾸는 날 이 상수가 조용히 틀려진다 — 각자 자기 몫만 든다.
 *
 * 제목→검색(12)보다 **좁다.** 사용자가 실기기에서 지적한 것이 정확히 이것이다:
 * 검색과 카테고리는 한 덩어리로 읽혀야 하는데 제목-검색보다 더 벌어져 있었다.
 */
export const SEARCH_TO_RAIL_GAP = spacing[8]
