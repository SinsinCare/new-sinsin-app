...
import {
  CommunityLibraryTabs,
  LIBRARY_TABS as TABS,
  type LibraryTab,
} from "../components/community/CommunityLibraryTabs"
...
/** Server-scoped, paginated personal activity; independent of loaded feed pages. */
export function CommunityLibraryScreen() {
  ...
  */
  const {
    ...
    nextPageError,
  } = useCommunityPosts({ library: activeTab })
  const { blockedAuthors, blockUser } = useBlockedUsers()
  const { data: profile, isLoading: isProfileLoading } = useMyPageProfile()
  ...
  return (
    ...
    >
      ...
      <CommunityProfileSummary
        name={
          profile?.nickName ??
          (isProfileLoading ? undefined : t("community.library.myProfile"))
        }
