...
4. 온보딩 제출 성공 직후 Figma 기반 완료 화면을 표시한다. 문구는 사용자의 확정대로 `회원가입이 완료되었습니다!`, CTA는 `시작하기`를 그대로 사용한다.
...
- `onboarding_completion_cta_pressed`: 사용자가 `시작하기`를 눌러 홈으로 이동함
