...
- [x] G1: Recent history round trips preserve destination semantics; legacy and invalid persisted data are handled; the live suggestion hook never exposes a previous input's options.
  ...
  EXPECT: /Tests:.*passed/
  EVIDENCE: exit=0; shell=/bin/sh; cwd=/Users/seongminhan/workspace/sinsin_dev/sinsin-rn; path=8030ff6970d9/24 entries; output=Time:        0.744 s, estimated 2 s | Ran all test suites matching /tests\/restaurantSearchRecent.test.ts/i.
...
Scope claim map-search-2026-09-08/GATES was released after reverify. Approval records use /tmp/sinsin-search-unlazy-approved because the default home directory is outside the writable sandbox.

Parent follow-up: the nested destination/delete Pressables were separated into sibling controls with a minimum delete touch target. After this change the parent reran the executable gates and full TypeScript check. Simulator showed distinct destination and delete accessibility buttons; deleting the test restaurant stayed on the search page and persisted after leaving/reopening. Region and restaurant replay plus filter integration were also exercised; see REVIEW.md.
