...
(각 절 끝에 출처). 2026-09-01 같은 문서 5개를 다시 확인 — 절차 변화 없음. 실제 콘솔
스크린샷으로 한국어 UI 경로를 대조해 아래에 병기했다.

**한국어 UI 주의:** ASC 사이드바 **수익화** 섹션의 **「앱 내 구입」 메뉴는 소모성·
비소모성용**이다. 자동 갱신 구독은 그 바로 아래 **「구독」** 메뉴에서 만든다.
Play Console 은 **Play를 통한 수익 창출 → 제품 → 정기 결제** 가 맞는 자리다.
...
> **실제 생성 결과(2026-09-01)는 권장안과 다르다.** iOS 는
> `sinsin_premium_monthly_v1`(1개월) · `sinsin_premium_annual_v1`(1년) — 둘 다
> 심사 준비됨. 안드로이드는 상품 **하나**(`premium_monthly`)에 기본 요금제
> `0`(매월) · `1`(매년) 두 개를 활성화한 구조다(구글의 현행 권장 모델).
> RevenueCat 에서 안드 상품은 `premium_monthly:0` / `premium_monthly:1` 로
> import 된다. 앱은 어차피 `$rc_monthly`/`$rc_annual` 만 보므로 코드 영향은 없다.
> 아래 절차의 `premium_monthly`/`premium_yearly` 표기는 이 실제 ID 로 읽을 것.
