...
const SCAN_DIRS = ["src/features/medication", "src/features/home", "src/features/settings"]
...
const namespaces: Record<string, Flat> = Object.fromEntries(
  fs.readdirSync(LOCALE_DIR).filter((f) => f.endsWith(".json")).map((f) => [f.replace(/\.json$/u, ""), flatten(JSON.parse(fs.readFileSync(path.join(LOCALE_DIR, f), "utf8")))]),
)
...
function placeholders(ns: Flat, key: string): string[] | null {
  const values = [ns[key], ns[`${key}_one`], ns[`${key}_other`]].filter((v): v is string => typeof v === "string")
  if (!values.length) return null
  return [...new Set(values.flatMap((v) => [...v.matchAll(/\{\{\s*([\w.]+)\s*\}\}/gu)].map((m) => m[1]!)))]
}
...
describe("i18n 자리표시자와 t() 인자", () => {
  ...
  for (const dir of SCAN_DIRS) {
    for (const file of sourceFiles(path.join(ROOT, dir))) {
      ...
      const fileNamespaces = [...src.matchAll(/useTranslation\(\s*"(\w+)"/gu)].map((m) => m[1]!)
      const candidates = fileNamespaces.length ? fileNamespaces : Object.keys(namespaces)
      for (const m of src.matchAll(/\bt\(\s*"([\w.]+)"\s*,\s*\{([^}]*)\}/gu)) {
        ...
        for (const nsName of candidates) {
          ...
          if (missing.length) problems.push(`${path.relative(ROOT, file)}: t("${key}") 에 ${missing.map((p) => `{{${p}}}`).join(", ")} 이 비었다 (넘긴 값: ${names.join(", ") || "없음"})`)
        }
      }
    }
  }
  ...
  it("검사가 헛돌지 않는다 — 실제 호출을 세었고, 대조군은 잡힌다", () => {
    ...
    expect(placeholders({ "x_one": "{{taken}}회", "x_other": "{{taken}}회" }, "x")).toEqual(["taken"])
  ...
  it("복수형 키는 {{count}} 로 쓴다 — 다른 이름은 count 를 넘겨도 채워지지 않는다", () => {
    ...
    for (const [nsName, ns] of Object.entries(namespaces)) {
      for (const [key, value] of Object.entries(ns)) {
        ...
        for (const p of [...value.matchAll(/\{\{\s*([\w.]+)\s*\}\}/gu)].map((x) => x[1]!)) {
